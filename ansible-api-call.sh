...
all_args=("$@")
template_type=$1                     #example: workflow_job_templates | job_templates
template_id=$2                       #example: 40 | 11
json_data=./ansible-api-call-data.json

#json_data=("${all_args[@]:2}") #example: "{"extra_vars": {"P1": "Colgan"}}"
#example usage:
#  bash ./ansible-api-call.sh job_templates 11
#  bash ./ansible-api-call.sh workflow_job_templates 40
...
echo;echo;echo "[*] retreiving template info"
curl -s -k -H "Authorization: Bearer $(jq -r .token ansible-api-token.json)" -H "Content-Type: application/json" -X GET "${tower_api}/${template_type}/"

## (OPTIONAL) use token to query the job|workflow template details
echo;echo;echo "[*] retreiving template info"
curl -s -k -H "Authorization: Bearer $(jq -r .token ansible-api-token.json)" -H "Content-Type: application/json" -X GET "${tower_api}/${template_type}/${template_id}/launch/"
#output example
...
echo;echo;echo "[*] invoking template execution"
curl -s -k -H "Authorization: Bearer $(jq -r .token ansible-api-token.json)" -H "Content-Type: application/json" -X POST  -d @$json_data "${tower_api}/${template_type}/${template_id}/launch/"
#output example
